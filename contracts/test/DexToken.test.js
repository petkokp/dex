...
contract('Dex token', async (accounts) => {
  ...
  it('should transfer token', async () => {
    ...
    await tokenInstance.transfer(accounts[1], 5000, { from: accounts[0] });

    const firstAccountBalance = await tokenInstance.balanceOf(accounts[0]);

    const secondAccountBalance = await tokenInstance.balanceOf(accounts[1]);
  ...
  it('should mint tokens', async () => {
    ...
    const thirdAccountBalance = await tokenInstance.balanceOf(accounts[2]);
  ...
  it('should deposit', async () => {
    ...
    await token.transfer(pool.address, 1000, { from: accounts[0] });
